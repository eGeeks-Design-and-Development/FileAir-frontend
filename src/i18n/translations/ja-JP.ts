export default {
  ...
  "home.bullet.a.description": "File Airをあなたのマシンでホストしましょう。",
  ...
  "admin.config.config-file-warning.description": "設定ファイル設定で済みのFile AirはUI を使用して設定を変更することはできません。",
  ...
  "admin.config.general.app-url.description": "File Airで利用できるURL",
  ...
  "admin.config.cache.redis-enabled.description": "通常、File Air は情報をメモリ内にキャッシュします。複数の File Air インスタンスを実行する場合は、インスタンス間でキャッシュを共有するために Redis キャッシュを有効にする必要があります。",
